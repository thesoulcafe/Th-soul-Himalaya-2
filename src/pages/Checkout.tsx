...
const handleOrder = async () => {
    ...
    try {
      if (paymentMethod === 'reserve') {
        ...
        const orderResult = await fetch(`${API_BASE}/api/razorpay/order`, {
          method: 'POST',
                      ...
                      {cart.map((item) => (
                        ...
                        >
                          <div className="h-16 w-16 md:h-20 md:w-20 rounded-2xl overflow-hidden shadow-lg shrink-0 border border-white/50">
                            <img src={(item.name || "").toLowerCase().includes("valley of shadows") ? "https://i.postimg.cc/wMSWmFKB/IMG-1095.webp" : item.image} alt={item.name} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" referrerPolicy="no-referrer" />
                          </div>
