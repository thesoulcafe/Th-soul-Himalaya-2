...
import { Plus, Minus, Flower2, Sun, Moon, Wind, Heart, Sparkles, CheckCircle2, Edit2, Clock, Zap, Calendar, ChevronDown, Star, Stars, ShoppingCart, ArrowRight, Share2 } from 'lucide-react';
...
export default function Meditation() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      DEFAULT_MEDITATION.forEach(def => {
        ...
        if (!exists) {
          combined.push({ ...def, originalType: 'meditation' });
        }
      });
                  ...
                  <CardContent className="p-8 flex-grow flex flex-col">
                    ...
                    {pkg.slots && pkg.slots.length > 0 && (
                      <div className="mb-6 p-4 rounded-2xl bg-forest/[0.03] border border-forest/5" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button 
                          onClick={() => navigate(`/meditation/${pkg.id}/book`)}
                      ...
                      {(() => {
                        ...
                        const handleBookAction = () => {
                          if (pkg.slots && pkg.slots.length > 0) {
                            navigate(`/meditation/${pkg.id}/book`);
                            return;
                          }
            ...
            >
              ...
              <div className="relative w-full h-[400px] md:h-[600px] shrink-0 overflow-hidden bg-forest">
                ...
                <div className="absolute bottom-10 left-10 right-10 text-white z-10">
                  ...
                  >
                    <span className="font-fluid text-2xl md:text-3xl text-terracotta drop-shadow-md mb-2 block text-center md:text-left">Inner Peace</span>
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-4 text-white/70 text-xs font-bold uppercase tracking-widest">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{selectedPackage.rating || '4.8'} / 5.0</span>
                    </div>
                    <div className="w-1 h-1 rounded-full bg-terracotta" />
                    <span>{selectedPackage.focus || 'Zen'}</span>
                  </div>
                </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 w-full lg:w-auto">
                          ...
                          <Button 
                            onClick={() => {
                              ...
                              globalAddToCart({
                                id: `meditation-${selectedPackage.id}${slotIndex !== undefined ? `-slot-${slotIndex}` : ''}`,
